...
def parse(resp, tweet_data):
    ...
    for tweet in tweets:
        ...
        try:
            ...
            tweet_info["tweet_text"] = tweet_info["tweet_text"].replace("\n", "")
            tweet_info["tweet_text"] = tweet_info["tweet_text"].replace(",", ".")
        ...
        try:
            ...
            # Remove quotation marks around "likes" if they exist
            tweet_info["likes"] = tweet_info["likes"].replace(",", "")
        except AttributeError:
...
stage_data(conn, table_name, tweet_data)

# Load the data into the table
load_data(conn, table_name)
